...
/// Get file name
#define FILENAME \
    (strrchr(__FILE__, FILESEP) ? \
    strrchr(__FILE__, FILESEP) + 1 : __FILE__)
...
#define RGM_LOG(level, msg) \
    ...
    BOOST_LOG_SEV(slg, level) << msg ; fflush(stdout); \
} while (0)
#endif
...
#define RGM_CHECK(conditions, level) \
    ...
    if (!(conditions)) { \
    BOOST_LOG_SCOPED_THREAD_TAG("Tag", FILENAME); \
    BOOST_LOG_SEV(slg, level) << "Check Failed: " <<  #conditions << "("\
    << FILENAME << ": " <<  __LINE__ << ")" ; fflush(stdout); \
    if ( level > error ) {\
    exit(0);\
}\
} \
